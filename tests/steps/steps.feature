...
Then('I should see the not found page', async () => {
  const notFoundContent = await page.locator('[class*="not-found"], [class*="notfound"]').first();
...
Then('the footer should contain copyright information', async () => {
  ...
  expect(text?.toLowerCase() || '').toMatch(/copyright|©|rights reserved/i);
});
...
Then('pricing plans should be properly aligned', async () => {
  ...
  expect(['grid', 'flex', 'block']).toContain(style);
});
